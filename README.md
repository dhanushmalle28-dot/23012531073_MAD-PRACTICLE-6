...
### MainActivity
1.  Design the layout for `MainActivity` as shown in the provided screenshot.
2.  Implement a frame-by-frame animation on the `ImageView` to cycle through different images of an alarm clock.
3.  Use an `<animation-list>` in a drawable resource for the frame-by-frame animation.
